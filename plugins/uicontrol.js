		...
		UiControl.prototype = {
		  ...
		  exportsModuleApis: function(){
		    ...
		    this.dataset.privateApis[api.privateApi.hideUiControl] = function(data,options){
		      ...
		      that.dataset.isControlShowing = false;
		    }
		    this.dataset.privateApis[api.privateApi.showUiControl] = function(data, options){
		      ...
		      if (!that.dataset.isControlShowing &&
		        that.context.msg.run(api.publicApi.getPlayerState) !==0
		      ) {
		        that.context.msg.broadcast(api.privateApi.updateProgress);
		      }
		      that.dataset.isControlShowing = true;
		    }
