	...
	/***/ function(module, exports, __webpack_require__) {
		...
		UiPlayerAd.prototype = {
		  ...
		  mute: function(){
		    ...
		    this.context.msg.broadcast(api.publicApi.mute, {showTips: false, broadcast: false});
		    this.context.msg.broadcast(api.eventApi.onAdMuteChange, {
		      mute: true
		    });
		  },
		  unMute: function(){
		    this.dataset.$volume.removeClass(this.dataset.muteClass);
		    this.context.msg.broadcast(api.publicApi.unMute, {showTips: false});
		    this.context.msg.broadcast(api.eventApi.onAdMuteChange, {
		      mute: false
		    });
		  },
		  ...
		  skipAd: function(){
		    ...
		    // if (this.dataset.volume!==0) this.unMute({showTips: false});
		  ...
		  addEventListener: function(){
		    ...
		    this.dataset.eventsList[util.getUniqueMsgKey(api.eventApi.onSectionTimeupdate)] = function(){
		      ...
		      var showTimes = that.dataset.duration - currenttime;
		      if (showTimes<0) return;
		      that.dataset.$time.html( showTimes );
		    }
		    ...
		    var whenCloseAdClick = function(){
		      ...
		      if (filmDuration && filmDuration>0 && filmDuration<180){
		        ...
		        if(that.isSkipAdButtonShow()){
		          ...
		          that.context.msg.broadcast(api.privateApi.onCloseAdClick);
		        }
		      }
		    };
		    ...
		    this.context.msg.on(api.eventApi.onError, function(data){
		      ...
		      that.context.msg.broadcast(api.privateApi.clearFocusVideoUrl);
		      that.hide();
		    });
		  },
		  exportsModuleApis: function(){
		    ...
		    this.dataset.privateApis[api.privateApi.adMuteStateUpdate] = function(data){
		      if (data && data.hasOwnProperty('mute')) {
		        if (data.mute){
		          that.mute();
		        }else{
		          that.unMute();
