...
*/
	...
	/***/ function(module, exports, __webpack_require__) {
		...
		var htmlstr = __webpack_require__(36);
		// htmlstr = htmlstr.replace(/txpdiv/g, 'div');
		var svgstr = __webpack_require__(37);
		var $ = Txplayer.$;
		...
		HtmlFrame.prototype = {
		  ...
		  write: function(){
		    ...
		    if (useSVG) {
		      var tmp = document.createElement('txpdiv');
		    }
		    var renderData = {
		      useSVG: useSVG,
		      ie: util.browser.ie,
		    ...
		    var errMsg = ['HtmlFrame.write Error:'];
		    var errStack;
		    var that = this;
		  ...
		  initHDPlayerEvent: function(){
		    var that = this;
		    this.dataset.$playermod.on('click mouseenter mousemove', function(e){
		    ...
		    $(document).off('keydown.txplayer').on('keydown.txplayer', function(e){
		      if (e.which === 9){
		        that.dataset.$playermod.addClass(that.dataset.focusClass);
		      }
		    });
		  ...
		  addEventListener: function(){
		    ...
		    this.dataset.$playermod.on('click', function(e){
		      ...
		      if (elem.nodeName.toLowerCase()==='a' && action) {
		        if (action==='openTxVideo'){
		          var url = util.getTxVideoPageURLByVid(that.context.msg.run(api.publicApi.getVid));
		          if (url){
		            window.open(url);
		          }
		        }
		        else if(action==='openVIP'){
		    ...
		    this.context.msg.on(api.privateApi.onCloseAdClick, function(){
		      that.dataset.$playermod.removeClass(that.dataset.focusClass);
		    });
		  },
