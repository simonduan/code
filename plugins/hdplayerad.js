	...
	/***/ function(module, exports) {
		...
		HdPlayerAd.prototype = {
		  ...
		  requestCgi: function(options){
		    ...
		    // 已经请求过就不要再请求前贴了
		    if (options.adType===adtypeMap['前贴'] && that.dataset.hasRequestLoadingAdData){
		      return;
		    }
		    that.dataset.hasRequestLoadingAdData = true;
		  ...
		  startInsertAd: function(currentTime){
		    ...
		    // 开始请求中插广告，标记为正在播放广告，锁住控制栏进度条
		    this.dataset.isPlayingAd = true;
		    this.requestCgi({
		      adType: adtypeMap['中插'],
		      zcindex: 0
		    });
		  ...
		  skipAd: function(){
		    ...
		    this.dataset.isPlayingAd = false;
		    var type = this.context.msg.run(api.privateApi.getCurrentPlayListType);
		    this.context.msg.broadcast(api.privateApi.clearPlaylist,type);
		    // 中插广告跳过逻辑
		    if (type==='insertad'){
		      this.context.msg.broadcast(api.eventApi.onSectionEnded,{
		        isAllEnd: false,
		        playListTypeEnd: true,
		        playListType: 'insertad'
		      });
		      return;
		    }
		    if (type==='endingad'){
		    ...
		    this.context.msg.broadcast(api.publicApi.pause);
		    this.context.msg.broadcast(api.privateApi.clearFocusVideoUrl);
		    this.context.msg.broadcast(api.privateApi.clearPreloadVideoUrl);
		    if (type==='endingad') return;
		    this.context.msg.broadcast(api.privateApi.play);
		  },
		  getPlayAdJsonData: function(){
		    return this.dataset.adJson;
		  },
		  setControlShowAndFixed: function(){
		    // this.context.msg.broadcast(api.privateApi.showUiControl);
		    // this.context.msg.broadcast(api.privateApi.hidePlayerUiTools, 2000);
		    // this.context.msg.broadcast(api.privateApi.disableHideControl, true);
		  },
		  addEventListener: function(){
		    ...
		    this.dataset.eventsList[util.getUniqueMsgKey(api.eventApi.onSectionTimeupdate)] = function(data){
		      if (that.context.msg.run(api.privateApi.isPlayingAd)) {
		        // 播放广告过程中，检查是否在播放广告
		        if (data && data.playListType==='film'){
		    ...
		    // 最后一个贴片最后5秒开始请求后贴
		    this.dataset.eventsList[util.getUniqueMsgKey(api.eventApi.onSectionTimeupdate)] = function(data){
		      var type, currentTime, duration;
		      if (data && data.playListType==='film' && !that.dataset.hasRequestEndingAdData) {
		        type = that.context.msg.run(api.privateApi.getCurrentPlayListType);
		        if (that.context.msg.run(api.privateApi.isTheLastOfPlayList, type)){
		          currentTime = that.context.msg.run(api.publicApi.getCurrentTime);
		          duration = that.context.msg.run(api.publicApi.getDuration);
		    ...
		    this.dataset.eventsList[util.getUniqueMsgKey(api.eventApi.onSectionEnded)] = function(data){
		      ...
		      if (data.playListTypeEnd && data.playListType!=='film'){
		        ...
		        that.context.msg.broadcast(api.eventApi.onAdEnd, {
		    ...
		    this.dataset.eventsList[util.getUniqueMsgKey(api.eventApi.onPreloadError)] = function(data){
		      if ( !(data && data.preloadType==='endingad') ) return;
		      var key = api.eventApi.onSectionEnded + '.onPreloadError';
		      that.dataset.eventsList[key] = function(data){
		        that.context.msg.broadcast(api.eventApi.onSectionEnded,{
		          isAllEnd: true
		        });
		      };
		      that.context.msg.once(key, that.dataset.eventsList[key]);
		    }
		    // vid切换了 onVidChange
		    this.dataset.eventsList[util.getUniqueMsgKey(api.eventApi.onVidChange)] = function(data){
		      that.dataset.hasRequestLoadingAdData = null;
		      that.dataset.hasRequestGetinfo = null;
		    }
		    // vid没有变化，触发 onBeforeVideoPlay
		    this.dataset.eventsList[util.getUniqueMsgKey(api.eventApi.onBeforeVideoRePlay)] = function(data){
		      that.dataset.hasRequestLoadingAdData = null;
		      that.dataset.hasRequestGetinfo = null;
		    }
		    this.dataset.eventsList[util.getUniqueMsgKey(api.eventApi.onBeforeVideoPlay)] = function(data){
		      that.dataset.hasRequestLoadingAdData = null;
		      that.dataset.hasRequestGetinfo = null;
		    }
