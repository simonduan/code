		...
		UiPlayNext.prototype = {
		  ...
		  playTheNext: function(options){
		    ...
		    if (options.hasOwnProperty('connectionPlayType')) {
		      nextConfig.connectionPlayType = options.connectionPlayType
		    }
		    that.context.superMsg.run(api.publicApi.play, nextConfig);
		  },
		  bindMouseEvent: function(){
		    var that = this;
		    this.dataset.$next.on('mouseenter', function(){
		      that.dataset.$next.addClass(Txplayer.dataset.btnHoverClass);
		    }).on('mouseleave', function(){
		      that.dataset.$next.removeClass(Txplayer.dataset.btnHoverClass);
		    });
		  },
