...
*/
	...
	/***/ function(module, exports, __webpack_require__) {
		...
		var htmlstr = __webpack_require__(43);
		...
		UiControlPlay.prototype = {
		  ...
		  initPlayState: function(){
		    ...
		    if (state===1 || state===3){
		      ...
		      this.dataset.$playbtn.attr('data-status', 'replay');
		      this.dataset.$tips.html('重新播放');
		    }else{
		  ...
		  bindMouseEvent: function(){
		    ...
		    this.dataset.$playbtn.on('mouseenter', function(){
		      that.dataset.$playbtn.addClass(Txplayer.dataset.btnHoverClass);
		    }).on('mouseleave', function(){
		      that.dataset.$playbtn.removeClass(Txplayer.dataset.btnHoverClass);
		    });
		  },
		  addEventListener: function(){
		    ...
		    this.dataset.$playbtn.on(Txplayer.dataset.clickEventName, function(){
		      ...
		      if (that.dataset.$playbtn.attr('data-status')==='pause') {
		        that.context.superMsg.broadcast(api.publicApi.pause);
		        that.context.superMsg.broadcast(api.eventApi.onUserPausePlayer);
		      }else{
		        if (that.dataset.$playbtn.attr('data-status')==='replay'){
		          that.context.superMsg.broadcast(api.eventApi.onBeforeVideoRePlay);
		        }
		    ...
		    this.dataset.eventsList[util.getUniqueMsgKey(api.eventApi.onSectionEnded)] = function(data){
		      ...
		      that.dataset.$playbtn.attr('data-status', 'replay');
		      that.dataset.$tips.html('重新播放');
		    }
