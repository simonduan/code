...
*/
	...
	/***/ function(module, exports, __webpack_require__) {
		 ...
		 * 高清视频+3Video
		 */
		var htmlstr         = __webpack_require__(28);
		var $               = Txplayer.$;
		...
		HdPlayer.prototype = {
		  init: function(){
		    ...
		    this.setVolume({
		      volume: this.context.config.volume,
		      showTips: false
		    });
		  },
		  ...
		  // video handler
		  videoHelper: function(vd, apiName, args){
		    args = args || [];
		    if (vd && apiName && $.type(vd[apiName])==='function'){
		      util.showInfo('videoHelper apiName', apiName);
		      if (args && args.length) {
		        vd[apiName].apply(vd, args);
		      }else{
		        vd[apiName]();
		      }
		    }
		  ...
		  isFocusVideoBufferComplete: function(vd){
		    ...
		    return parseInt(video.duration) === parseInt(buffered);
		  },
		  // 兼容各个浏览器设置播放时间点
		  ...
		  setVideoPlayTime: function(video, time, autoplay){
		    ...
		    // 除了safari，全部自动播放
		    if (autoplay && !util.browser.safari) {
		      this.videoHelper(video, 'play');
		    }
		    var that = this, args = arguments;
		    if ( util.browser.safari && autoplay){
		      // 缓冲数据足够了
		      if (video && video.buffered && video.buffered.length && video.buffered.end(video.buffered.length-1) >= time){
		        this.videoHelper(video, 'play');
		      }else{
		        // 重试
		        setTimeout(function(){
		          that.setVideoPlayTime.apply(that, args);
		        },1000);
		      }
		    }
		  },
		  ...
		  setFocusVideoCurrentTime: function(time, autoplay){
		    ...
		    this.setVideoPlayTime(video, time, autoplay);
		  },
		  // 获取当前显示的视频的播放时间
		  ...
		  setPreloadVideoPlay: function(options){
		    ...
		    var preloadCurrentTime = options.currentTime || preloadVideo.currentTime;
		    if (!preloadVideo || !currentVideo) return;
		    if (!preloadVideo.currentSrc) {
		  ...
		  setPreloadVideoKeepSpaceToFocusVideo: function( cb ){
		    ...
		    var playPreloadVideo = function(){
		      // that.setVideoPlayTime(preloadVideo, focusVideo.currentTime, true);
		      // 预加载的视频直接播放
		      that.setPreloadVideoPlay({
		        notNeedUpdateIndex: true,
		        reload: false,
		        currentTime: focusVideo.currentTime
		      });
		    ...
		    if (!focusVideo || !focusVideo.currentSrc) return;
		    stopTimer();
		    ...
		    that.dataset.keepSpace_TIMER = setInterval(function(){
		      if (!preloadVideo || !preloadVideo.currentSrc) return;
		      if ( (focusVideo.duration - focusVideo.currentTime) < 5 && !focusVideo.paused ) {
		        that.videoHelper(focusVideo, 'pause');
		      }
		      that.setVideoPlayTime(preloadVideo, focusVideo.currentTime, false);
		      // preloadVideo.currentTime = focusVideo.currentTime;
		    ...
		    that.dataset.keepSpaceTimeout_TIMER = setTimeout(function(){
		      ...
		      stopTimer();
		      var errorMsg;
		      if (!preloadVideo || !preloadVideo.currentSrc) {
		  ...
		  lockAndPreloadNext: function(options){
		    ...
		    var msgKeyError = api.eventApi.onPreloadError + '.lockAndPreloadNext';
		    ...
		    var switchLock = function(){
		      ...
		      that.videoHelper(focusVideo, 'pause');
		      that.videoHelper(preloadVideo, 'play');
		      preloadVideo.isFocus = true;
		      preloadVideo.status = videoStatusMaps.PLAYING;
		      ...
		      var msgKeyOnerror = api.eventApi.onError + '.lockAndPreloadNext';
		      that.context.msg.on(msgKeyEnded, function(data){
		        if (data.playListTypeEnd && data.playListType) {
		          ...
		          that.videoHelper(focusVideo, 'play');
		    ...
		    // 广告播放失败
		    that.context.msg.off(msgKeyError);
		    // 中插广告素材加载失败
		    that.context.msg.once(msgKeyError, function(err){
		      if (err && err.type==='insertad'){
		        that.dataset.index = index;
		        if (err.videoTag){
		          err.videoTag.src = '';
		        }
		  ...
		  resetVideoStatus: function(){
		    this.dataset.$video.each(function(idx, videoTag){
		      ...
		      if (videoTag.currentSrc) {
		        videoTag.src = '';
		        $(videoTag).removeAttr('src');
		      }
		    });
		    this.updateVideoDisplay();
		    this.dataset.disablePreload = false;
		    util.showInfo('resetVideoStatus调用了','');
		  },
		  // 更新多个视频的交替显示和隐藏
		  ...
		  setVolume: function(options){
		    ...
		    // 初始化不触发音量变化事件
		    if ($.type(this.dataset.volume)!=='undefined' && volume!==this.dataset.volume) {
		  ...
		  mute: function(options){
		    options = options || {};
		    if (!options.hasOwnProperty('broadcast')) options.broadcast = true;
		    this.dataset.$video.each(function(idx, vd){
		      vd.muted = true;
		    });
		  ...
		  calcDownloadSpeed: function(){
		    ...
		    this.dataset.$video.on('progress', function(){
		      ...
		      var info = that.getFileInfoBySrc(this.currentSrc);
		      if (!info) {
		        return;
		      }
		      pushData({
		      ...
		      calcSpeed( bufferData[this.currentSrc], this.currentSrc );
		    });
		    this.dataset.$video.on('timeupdate', function(){
		      ...
		      if (buffered.end( buffered.length -1) === this.duration){
		        calcAverageSpeed(this.currentSrc);
		      }
		    });
		  },
		  startWaitingFinder: function(){
		    if (this.dataset.waitingFinder_TIMER) {
		      this.stopWaitingFinder();
		    }
		    var video = this.getFocusVideoTag();
		    if (!video) return;
		    var prevTime;
		    ...
		    this.dataset.waitingFinder_TIMER = setInterval(function(){
		      if (!video.isFocus){
		        video = that.getFocusVideoTag();
		      }
		      if (prevTime===video.currentTime && !(video.paused || video.ended) ) {
		  ...
		  addEventListener: function(){
		    ...
		    .on('ended', function(){
		      if (!this.isFocus) return;
		      var type = that.getCurrentType();
		      var isAllEnd = that.isTheLastOfPlayList() && type!=='insertad'
		      // 首先触发statechange
		      if (type==='film' && isAllEnd){
		        that.setPlayState(0);
		      }
		      // 再广播视频播放结束
		    ...
		    .on('error', function(e){
		      var currentType = that.getCurrentType();
		      var preloadType;
		      if (!this.isFocus) {
		        if ( that.dataset && that.dataset.playList && that.dataset.playList[that.dataset.index+1] ) {
		          preloadType = that.dataset.playList[that.dataset.index+1].type;
		        }
		        that.context.msg.broadcast(api.eventApi.onPreloadError,{
		          type: currentType,
		      ...
		      if (that.dataset.errorRetryTimes>5 || !this.currentSrc){
		        hasRetry = true;
		        delete that.dataset.errorRetryTimes;
		      }else{
		        that.videoHelper(this, 'pause');
		        try{
		          that.videoHelper(this, 'load');
		        }catch(err){
		          // 待上报
		          util.showInfo(err, err.stack);
		        }
		    ...
		    this.context.msg.on(api.privateApi.addUrls2Player, function(param, options){
		      ...
		      if (data.data && data.type && data.repalcePlayList===true) {
		        ...
		        // 分片数不一致
		          ...
		          that.dataset.playList = [];
		          // that.dataset.index = 0;
		          that.dataset.playList = data.data;
		        }
		    ...
		    this.context.msg.on(api.privateApi.clearPreloadVideoUrl, function(data, options){
		      that.clearPreloadVideoUrl();
		    });
		    this.context.msg.on(api.eventApi.onBeforeVideoPlay, function(data, options){
		      that.setPlayState(-1);
		    });
	
		    this.context.msg.on(api.eventApi.onSectionPlaying, function(){
		      that.startWaitingFinder();
		    });
		    this.context.msg.on(api.eventApi.onSectionEnded, function(data,options){
		      if (data && data.isAllEnd) {
		  ...
		  exportsModuleApis: function(){
		    ...
		    this.dataset.privateApis[api.privateApi.resetVideoStatus] = function(data, options){
		      that.resetVideoStatus();
		    };
