...
*/
	...
	/***/ function(module, exports, __webpack_require__) {
		...
		var htmlstr = __webpack_require__(82);
		...
		UiVolume.prototype = {
		  init: function(){
		    this.dataset.volumeKey = Txplayer.dataset.localStorageKey.volumeHistory;
		    this.write();
		    this.setVolumeByCache();
		    this.onVolumeUpdate();
		  ...
		  setVolumeByCache: function(){
		    var volume = this.getData();
		    var that = this;
		    if ($.type(volume)==='number'){
		      if ( this.context.superMsg.listenerCount(api.publicApi.setVolume) ){
		        this.context.superMsg.broadcast(api.publicApi.setVolume, volume);
		      }else{
		        this.context.superMsg.on(api.eventApi.onReady, function(){
		  ...
		  getData: function(){
		    var datastr = util.getData(this.dataset.volumeKey);
		    var volume = parseInt(datastr);
		    if (!isNaN(volume) && $.type(volume)==='number'){
		      return volume;
		    }
		    return;
		  },
		  setData: function(val){
		    util.setData(this.dataset.volumeKey, val);
		  },
		  ...
		  volumeDrag: function(){
		    ...
		    var calcVolumeByMousePosition = function(e){
		      ...
		      that.context.superMsg.broadcast(api.publicApi.setVolume, {
		        volume: volume,
		        showTips: true
		      });
		    };
		  ...
		  addEventListener: function(){
		    ...
		    this.dataset.eventsList[util.getUniqueMsgKey(api.eventApi.onVolumeChange)] = function(data){
		      that.onVolumeUpdate();
		    };
		    // 广告播放结束
		    this.dataset.eventsList[util.getUniqueMsgKey(api.eventApi.onAdEnd)] = function(data){
		      that.setVolumeByCache();
		    };
