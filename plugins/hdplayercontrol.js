...
*/
	...
	/***/ function(module, exports, __webpack_require__) {
		...
		var qvsec = __webpack_require__(20);
		var $ = Txplayer.$;
		var TizenAuth = __webpack_require__(21);
		var playerdefine = __webpack_require__(23);
		...
		GetVideoInfo.prototype = {
		  ...
		  getHDVideoUrlByVid: function(options, retryOptions){
		    ...
		    // retry
		    var retryFN = arguments.callee;
		    if (retryOptions.isRetry && retryOptions.cgi) {
		      cgi = retryOptions.cgi;
		    }
		    if (retryOptions.isRetry && retryOptions.defer) {
		      defer = retryOptions.defer;
		    }
		    ...
		    .done(function(data){
		      ...
		      if (checkData===1) {
		        ...
		        data.iRetCode = checkCode.iRetCode;
		    ...
		    .fail(function(xhr, err){
		      var retryConfig;
		      if ( !(retryOptions && retryOptions.isRetry) ) {
		        retryConfig = {
		          defer: defer,
		          isRetry: true,
		          cgi: location.protocol==='https:'?'https://sec.video.qq.com/p/bkh5vv.video/getinfo?callback=?':'http://bkh5vv.video.qq.com/getinfo?callback=?'
		        };
		        retryFN.call(that, options, retryConfig)
		      }else{
		        defer.reject({
		          code: 444
		        });
	...
	/***/ function(module, exports, __webpack_require__) {
		...
		var $ = Txplayer.$;
		var htmlstr = __webpack_require__(22);
	...
	/***/ function(module, exports, __webpack_require__) {
		 ...
		 * html5 高清播放器控制器
		 */
		var GetVideoInfo  = __webpack_require__(19);
		var $             = Txplayer.$;
		var api           = Txplayer.apiList;
		var util          = Txplayer.util;
		var definitionMap = __webpack_require__(23).definitionMap;
		var errcode = __webpack_require__(32);
		...
		HdPlayerControl.prototype = {
		  ...
		  init: function(){
		    ...
		    this.dataset.setDefinitionDoneKey = api.eventApi.onSectionPlaying + '.switch_definition';
		    ...
		    this.dataset.connectionPlayType = 0;
		  ...
		  requestVideoUrlsByVid: function(options){
		    ...
		    this.getinfo.getHDVideoUrlByVid(config).done(function(videoFirstPartUrl, videosData, DefinitionList, getinfoJSON){
		      ...
		      that.dataset.canPlayTime = null;
		      if (getinfoJSON && getinfoJSON.preview &&
		        (getinfoJSON.exem===1 || getinfoJSON.iRetCode===83)
		      ) {
		        that.dataset.canPlayTime = getinfoJSON.preview;
		      }
		      that.context.msg.broadcast(api.eventApi.onGetVideoUrlSuccess, videoFirstPartUrl);
		      that.msg.broadcast('onTrialDataUpdate', {
		        code: getinfoJSON.iRetCode,
		        trailTime: getinfoJSON.preview
		      });
		    }).fail(function(data){
		  ...
		  loadVideoUrls: function(options){
		    ...
		    this.context.msg.once(msgName, function(videoFirstPartUrl){
		      ...
		      if (options.connectionPlayTime) {
		        that.seekTo({
		          connectionPlayTime:options.connectionPlayTime
		        });
		      }else if (options.playStartTime) {
		        that.seekTo({
		          playStartTime:options.playStartTime
		        });
		      }
		    });
		  ...
		  setDefinition: function(defn){
		    ...
		    this.dataset.prevDefinition = this.getDefinition();
		    ...
		    this.context.msg.once(api.eventApi.onPreloadListChange+'.switch_definition', function(url){
		      ...
		      var urlData;
		      var currentTime = that.context.msg.run(api.privateApi.getFocusVideoCurrentTime);
		      var seekToInfo;
		      // index=0,可能是mp4一个分片的情况
		      if (index===0){
		        seekToInfo = that.getVideoSeekToInfo(currentTime);
		        if (seekToInfo.hasOwnProperty('index') && seekToInfo.hasOwnProperty('prevDurtion')){
		          index = seekToInfo.index;
		          currentTime = currentTime - seekToInfo.prevDurtion;
		      ...
		      if ($.type(urlData)==='string'){
		        ...
		        urlData.autoplay = true;
		        urlData.currentTime = currentTime;
		        urlData.updateIndex = index;
		        urlData.type = 'film';
		      }
		      that.stopSetDefinition();
		      that.context.msg.broadcast(api.privateApi.setFocusVideoUrl, urlData);
		      that.context.msg.once(that.dataset.setDefinitionDoneKey, function(){
		        that.dataset.hasSetDefinitionRetry = false;
		        that.stopSetDefinition();
		        that.dataset.isDefinitionSwitching = false;
		      ...
		      that.dataset.setDefinitionTimeOut_TIMER = setTimeout(function(){
		        that.stopSetDefinition();
		        util.showInfo('切换清晰度失败','');
		        that.dataset.isDefinitionSwitching = false;
		        that.context.msg.broadcast(api.eventApi.onSetDefinitionFail,{});
		        // 切换清晰度失败，重试
		        if (that.dataset.prevDefinition && !that.dataset.hasSetDefinitionRetry) {
		          that.dataset.setDefinitionRetry_TIMER = setTimeout(function(){
		            that.stopSetDefinition();
		            that.setDefinition(that.dataset.prevDefinition);
		            that.dataset.hasSetDefinitionRetry = true;
		          },1500);
		        }
		      },20000);
		    });
		  ...
		  stopSetDefinition: function(){
		    this.dataset.isDefinitionSwitching = false;
		    // this.context.msg.broadcast(api.privateApi.hideUiTips);
		    this.context.msg.off(this.dataset.setDefinitionDoneKey);
		    ...
		    if (this.dataset.setDefinitionRetry_TIMER){
		      clearTimeout(this.dataset.setDefinitionRetry_TIMER);
		      this.dataset.setDefinitionRetry_TIMER = null;
		    }
		  },
		  // 获取清晰度列表
		  ...
		  seekTo: function(options){
		    ...
		    if (state===0){
		      util.showError('播放已结束，不能seek');
		      return;
		    }
		    if (state===-1){
		      util.showError('播放状态未初始化，不能seek');
		      return;
		    }
		    this.stopSetDefinition();
		    var seekTime;
		    if ( $.type(options)==='object' ){
		      seekTime = options.time || options.connectionPlayTime || options.playStartTime;
		    }else if ( $.type(options)==='number' ){
		      seekTime = options;
		      options = {};
		    }
		    util.showInfo('seekTo', seekTime);
		    if ( $.type(seekTime)!=='number' ) {
		      util.error('seekTo参数错: 参数必须为number');
		      return;
		    }
		    var duration = this.getDuration();
		    var index = this.context.msg.run(api.privateApi.getPlayListIndex, 'film');
		    if (seekTime >= duration) {
		      util.showError('seekTo参数错误: seekTime['+seekTime+']不能超过视频的时长['+duration+']','');
		      return;
		    }
		    var seekInfo = this.getVideoSeekToInfo(seekTime);
		    var that = this, textTime, config;
		    this.context.msg.broadcast(api.eventApi.onSeekStart);
		    // seek之后，开始播放，标识seek完成
		    ...
		    if (index === seekInfo.index) {
		      ...
		      if (this.dataset.videoPreloadList[seekInfo.index]) {
		        config = this.dataset.videoPreloadList[seekInfo.index];
		    ...
		    if (options && options.showTips) {
		      textTime = '<a>' + util.formatPlayTime(seekTime) + '</a>';
		      that.context.msg.broadcast(api.privateApi.showUiTips, {
		        text: '您上次观看至 '+textTime+' 处，正在为您续播',
		        time: 3,
		        hideClose: true
		      });
		    }else if (options.connectionPlayTime){
		      textTime = '<a>' + util.formatPlayTime(options.connectionPlayTime) + '</a>';
		      that.context.msg.broadcast(api.privateApi.showUiTips, {
		        text: '您上次观看至 '+textTime+' 处，正在为您续播',
		        time: 3,
		        hideClose: true
		      ...
		      // textTime = '<a>' + util.formatPlayTime(options.playStartTime) + '</a>';
		      that.context.msg.broadcast(api.privateApi.showUiTips, {
		        text: '已为您跳过片头',
		        time: 3,
		        hideClose: true
		    ...
		    // 弹幕id
		    if (options.bulletid) {
		      that.context.msg.broadcast(api.eventApi.onBarrageSeekTime, {
		  ...
		  play: function(options){
		    util.showInfo('player.play is Called```', arguments);
		    var isPlayNext = function(){
		      if ($.type(options)==='string' || ($.type(options)==='object' && options.vid)){
		        return true;
		      }
		      return false;
		    };
		    var that = this;
		    // 当前在播放广告
		    if (this.context.msg.run(api.privateApi.isPlayingAd) && !isPlayNext() ){
		    ...
		    if (options.hasOwnProperty('cid'))
		      this.dataset.cid = options.cid;
		    if (options.hasOwnProperty('columnId'))
		    ...
		    if (options.playStartTime)
		      ...
		      this.dataset.playStartTime = 0;
		    if (options.hasOwnProperty('connectionPlayType'))
		      this.dataset.connectionPlayType = options.connectionPlayType;
		    else
		      this.dataset.connectionPlayType = 0;
		    util.showInfo('connectionPlayType', this.dataset.connectionPlayType);
		    this.dataset.hasVideoPlayed = false;
		    this.dataset.hasLoadVideoUrl = false;
		    this.dataset.isLoadingAdEmpty = null;
		    this.context.msg.broadcast(api.privateApi.resetVideoStatus);
		  ...
		  getSkipPreludeTime: function(){
		    ...
		    playStartTime = parseInt(playStartTime);
		  ...
		  addEventListener: function(){
		    ...
		    this.context.msg.on(api.eventApi.onLoadingadIsEmpty, function(){
		      ...
		      config.connectionPlayTime = that.getConnectionPlayTime();
		    ...
		    this.context.msg.on(api.eventApi.onSectionPlay, function(data){
		      ...
		      if (that.dataset.isLoadingAdEmpty) return;
		      var playStartTime = that.getSkipPreludeTime();
		      var connectionPlayTime = that.getConnectionPlayTime();
		      if ($.type(playStartTime)!=='number' || playStartTime < 0) return;
		      if ( $.type(connectionPlayTime)==='number' && connectionPlayTime ){
		        that.seekTo({
		          connectionPlayTime: connectionPlayTime
		        });
		      }else if ( $.type(playStartTime)==='number' && playStartTime ) {
		        that.seekTo({
		    ...
		    this.context.msg.on(api.eventApi.onInsertAdDataReady, function(data){
		      ...
		      var tmpInsertInfo;
		      var insertIndex = insertInfo.index + 1;
		      // 如果插入节点在分片切换处
		      if (data.insertTime && data.insertTime>15){
		        tmpInsertInfo = that.getVideoSeekToInfo(data.insertTime-15);
		        if (tmpInsertInfo && tmpInsertInfo.index !== insertInfo.index){
		          insertIndex -= 1;
		        }
		      }
		    ...
		    this.context.msg.on(api.eventApi.onVideoInterruptByError, function(){
		      if(that.dataset.getVkeyErrorData && that.dataset.getVkeyErrorData.em===83){
		        ...
		        that.dataset.isSwitchTo1080pAfterLogin = true;
		        that.pause();
		      }
		    });
		    ...
		    // 付费试看
		    that.msg.on('onTrialDataUpdate', function(data){
		      var showTips = function(){
		        if (that.context.msg.run(api.privateApi.isUiTipsShowing)) {
		          return;
		        }
		        var text = '可试看{$time}，去腾讯视频观看完整版 <a href="javascript:;" data-action="openTxVideo">立即前往</a>';
		        text = '可试看{$time}，观看完整影片请<a href="javascript:;" data-action="openVIP">开通VIP</a>';
		        var time = data.trailTime;
		        if (time<60) {
		          time += '秒';
		        }else{
		          time = parseInt(time/60,10) + '分钟';
		        }
		      ...
		      var stopTips = function(){
		        if (that.dataset.trialMovie_TIMER) {
		          clearInterval(that.dataset.trialMovie_TIMER);
		          that.dataset.trialMovie_TIMER = null;
		          that.context.msg.broadcast(api.privateApi.hideUiTips);
		        }
		      };
		      if (data && (data.code+'') === '83') {
		        showTips();
		        that.dataset.trialMovie_TIMER = setInterval(function(){
		          showTips();
		        },5000);
		      }else{
		        stopTips();
		    ...
		    Txplayer.msg.on(api.eventApi.onAfterLogin, function() {
		      if (that.dataset.isSwitchTo1080pAfterLogin) {
		        that.setDefinition('fhd');
		        that.dataset.isSwitchTo1080pAfterLogin = null;
		      }
		    });
	
		    // 观看时长控制
		    that.context.msg.on(api.eventApi.onSectionTimeupdate, function(data){
		      if ( !(data && data.playListType==='film')) return;
		      if ( !that.dataset.canPlayTime) return;
		      var currentTime = that.getCurrentTime();
		      if (currentTime && currentTime>that.dataset.canPlayTime) {
		        that.context.userMsg.broadcast(api.eventApi.showUIVipGuide,{
		          trialFinish: true
		        });
		        that.pause();
		    ...
		    that.context.msg.on(api.eventApi.onSectionPlaying, function(data){
		      if ( !(data && data.playListType==='film')) return;
		      if ( !that.dataset.canPlayTime) return;
		      var currentTime = that.getCurrentTime();
		      if (currentTime && currentTime>that.dataset.canPlayTime) {
		  ...
		  exportsModuleApis: function(){
		    ...
		    this.dataset.moduleApis[api.publicApi.play] = function(data){
		      if (!data) data = {};
		      if ($.type(data)==='object' && !data.hasOwnProperty('nextVid')) data.nextVid = that.dataset.nextVid;
		      that.play(data);
		    }
		    ...
		    this.dataset.moduleApis[api.publicApi.setSmallWindowMode] = function(data, options){
		      if (data){
		        that.dataset.isSmallWindowMode = true;
		        that.context.msg.broadcast(api.privateApi.hideUiTips);
		        that.context.msg.broadcast(api.eventApi.onSmallWindowModeChange, {isSmallWindow: true});
		      }else{
		        that.dataset.isSmallWindowMode = false;
		        that.context.msg.broadcast(api.eventApi.onSmallWindowModeChange, {isSmallWindow: false});
		      }
		    };
		    this.dataset.moduleApis[api.privateApi.isSmallWindowMode] = function(data, options){
		      options.data = !!that.dataset.isSmallWindowMode;
		    };
		    ...
		    this.dataset.privateApis[api.privateApi.getConnectionPlayType] = function(data, options){
		      options.data = that.dataset.connectionPlayType;
		    }
