	...
	/***/ function(module, exports, __webpack_require__) {
		...
		UiSettings.prototype = {
		  ...
		  getHistorySettings: function() {
		    ...
		    var setTheaterMode = function(isTheater){
		      if ( !that.context.superMsg.listenerCount( api.publicApi.setTheaterMode ) ) {
		        that.context.superMsg.on(api.eventApi.pluginReady, function(data){
		          if (data && data.name==='UiToggleSidebar'){
		            that.context.superMsg.broadcast(api.publicApi.setTheaterMode, isTheater);
		          }
		        });
		      }else{
		        that.context.superMsg.broadcast(api.publicApi.setTheaterMode, isTheater);
